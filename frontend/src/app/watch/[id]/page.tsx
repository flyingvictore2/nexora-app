        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden md:flex flex-col gap-4 w-44 flex-shrink-0">
            ...
            <div className="space-y-2 text-xs text-gray-400">
              {content.releaseYear && <div className="flex items-center gap-2"><span>📅</span> {content.releaseYear}</div>}
              {content.duration && <div className="flex items-center gap-2"><Clock className="w-3 h-3" /> {formatDuration(content.duration)}</div>}
              {content.language && <div className="flex items-center gap-2"><Globe className="w-3 h-3" /> {content.language.toUpperCase()}</div>}
              {content.country && <div className="flex items-center gap-2"><span>🌍</span> {content.country}</div>}
              ...
              {tabs.map((t) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === t.id ? 'border-nexora-red text-white' : 'border-transparent text-gray-500 hover:text-gray-300',
                  )}
                >
                  {t.label}
                </button>
