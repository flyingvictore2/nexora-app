...
async function main() {
    ...
    const plans = await Promise.all([
        prisma.plan.upsert({
            where: { planType: client_1.SubscriptionPlan.FREE },
            create: {
                name: 'Gratis',
                planType: client_1.SubscriptionPlan.FREE,
                price: 0,
                currency: 'USD',
        ...
        prisma.plan.upsert({
            where: { planType: client_1.SubscriptionPlan.PREMIUM },
            create: {
                name: 'Premium',
                planType: client_1.SubscriptionPlan.PREMIUM,
                price: 12.99,
                currency: 'USD',
        ...
        prisma.plan.upsert({
            where: { planType: client_1.SubscriptionPlan.VIP },
            create: {
                name: 'VIP',
                planType: client_1.SubscriptionPlan.VIP,
                price: 19.99,
                currency: 'USD',
    ...
    const movies = [
        {
            ...
            isNew: true,
            isPublished: true,
            requiredPlan: client_1.SubscriptionPlan.FREE,
        },
        {
            ...
            isNew: true,
            isPublished: true,
            requiredPlan: client_1.SubscriptionPlan.PREMIUM,
        },
        {
            title: 'Noche Eterna',
    ...
    const animes = [
        {
            ...
            type: client_1.ContentType.ANIME,
    ...
    for (const m of [...movies, ...series, ...animes]) {
        ...
        if (!existing) {
            const c = await prisma.content.create({ data: m });
